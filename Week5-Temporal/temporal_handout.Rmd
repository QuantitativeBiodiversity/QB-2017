...
When using pipes, you use the output from the function to left to feed it into the next function to the right. 

To build the time-by-species matrix, we will also be using a `dplyr` function called `group_by`, which "splits" up the dataset using one or more variables.
After this, one can apply functions to the split dataset like `summarize()`, which can return summary statistics (e.g., mean, min, max).
You can also use the `spread()` function from `tidyr` on the split dataset to create a wide-format dataset from a long-format dataset. 
Let's give it a try:
...
# NW: Not sure what below was intended to do, but year can't be rownames because they repeat
...
When addressing questions related to temporal biodiversity, one must beaware of important features and assumption related to time series. 
For example, we discussed autocorrelation that can arise in a spatial context, 
...
abund <- rowSums(filter(abund.try, month, plot_id == 5)[,-c(1,2)])
