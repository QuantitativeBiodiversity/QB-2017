...
package.list <- c('vegan', 'tidyr', 'dplyr', 'codyn', 'ggplot2', 
      ...
      'TTR', 'xtable', 'multcomp', 'pander', 'png', 'grid', 'tseries')
...
The success of the Portal Project is based in part on insightful questions and the generation of data from a creative long-term experiment. 
In addition to generating a lot of quality data, it was criticial for the Protal Project data to be organized, accessible, and properly managed.  
One thing you may notice when looking at `portal` is that it has a lot of observations, almost 35,000 (and remmeber this is only 1977 - 2002).
...
The functions contained in these two packages (and others; see `apply()` and `aggregate()`) are fast and efficienct, and thus are preferred ways for manipulating data in R compared to other control-flow statements (e.g., for loops and while loops) that are commonly used in other programming languages. 
...
When using pipes, the output from the function to left is fed into the next function to the right. 
To build the time-by-species matrix, we will also use a `dplyr` function called `group_by`, which splits the dataset up using a single or multiple variables.
...
You can also use the `spread()` function from `tidyr` on the split dataset to convert parts of a long format dataset to a wide format dataset. 

In the following R chunk, we will use `count()` to sum the number of individuals belonging to each taxon while maintaining the grouping by year and plot_id.
In this case, the fill argument assigns a value of zero if there are no values for a given combination of groupings
...
Because many questions in biodiversity science involve time, we are going to provide a brief introduction to time series analysis.
Time series analysis involves statistical tools for detecting and decomposing trends in temporal data, and distinguishing this from other sources (e.g., seasonality, cycles, and error).
...
where ${\mu} is the mean of the time series ($Y$), ${\theta}$ is fitted parameter, and ${\epsilon}$ refers to errors associated with order (or lag) $q$. The full ARMA model can then be expressed as follows
...
Before that, we will aslo introduce some other commonly used techniques that are commonly used in the analysis of time series. 
We will demonstrate these tools using the abundance of rodents in a single site of the Portal Project.
...
We will then express the abundance data on a per hectare basis (0.25 * 4 = 1 ha).
We will also use the `ts()` function to make a time series object for our corrected data.
...
plot.ts(abund.ts, type = "l", ylab = "Rodent Abundance (#/hectrare)", xlab = "Time (year)", las = 1, ylim = c(0, 500))
```

### Smoothing

One technique that is commonly used, mostly for visualizing trends in a time series is *smoothing*. 
These types of techniques effectively remove noise from a data set, and aid in detecting signal in your dataset.
The simplest form of smoothing is the *simple moving average*. 
Not to be confused with the moving average procedure describd in the ARMA, the simple moving average calculates an unweighted arithmetic mean for *Y* across a window of past *n* observations.  
Let's give this a try uses the `SMA()` function in the `TTR` package. 
Try out the code below and play around with *n*.

```{r{}} 
abund.sm <- SMA(abund$nn, n = 5)
plot(abund.sm, type = "l", ylab = "Rodent Abundance (#/hectrare)", xlab = "Time (year)", las = 1, ylim = c(0, 500))
lines(abund$nn, col = "red")
legend(2, 450, col = c("black", "red"), lty = c(1,1), c("non-smooth", "smooth"), bty = "n", cex = 1)
```

One can also *exponentially smooth* a time series. 
Such procedures often use low-pass filters that apply exponentially less weight on observations over time. 
...
From the output, you will get an alpha parameter, which ranges from 0 to 1. 
...
# abund.hw$fitted
plot(abund.hw, xlab = "Time (year)", ylab = "Rodent Abundance (#/hectrare)", las = 1, main = NA)
legend(2, 400, col = c("black", "red"), lty = c(1,1), c("non-smooth", "smooth"), bty = "n", cex = 1)
```

### Decomposition of a time series
A time series can be broken down into different categories to help explain what is giving rise to patterns. 
The common categories of a time series *decomposition* are the trend, seasonal, cycle, and error.
...
In the following section we will use the `decompose()` function which decomposes the time series so that we can assess contributions from the overal trend, seasonality, and radom processes (i.e., error).
If seasonal trends are a nuisance in your study, there are ways to remove them, as done in the chunk below. 

```{r{}
abund.comp <- decompose(abund.ts) # moving average decomposition
plot(ab.comp) 
abund.adj <- abund.ts - abund.comp$seasonal # remove seasonality
plot.ts(abund.adj, ylab = "Rodent Abundance (#/hectrare)", xlab = "Time (year)", las = 1, col = "red", ylim = c(0, 500))
lines(abund.ts, col = "black")
legend(1977, 475, col = c("black", "red"), lty = c(1,1), c("with seasonality", "seasonality corrected"), bty = "n", cex = 1)
```
