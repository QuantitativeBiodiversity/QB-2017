...
However, manipulating long-format data for visualization and statistical analysis is often a challenge. 
...
The functions contained in these two packages (and others like `apply()` and `aggregate()`) are preferred for manipulating data in R because they are faster and more efficient than other control-flow statements (e.g., loops) that are commonly used in other programming languages. 
...
When using pipes, the output from the function to the left becomes the first argument of the function on the right. 
...
One goal of time series analysis is to detect and decompose trends in temporal data that are independent from other sources of variability (e.g., seasonality, cycles, and error).
...
Throughout the year, total rodent abundance may fluctuate widely in response to multiple stimuli, one of which is likely precipitation.
...
Specifically, the ACF tells us about the lags of the forecast errors and thus provides information for the MA component of the model. 
...
These are important questions for basic science, but are also important for topics in restoration ecology where legacy effects often impede management practices. 
...
So, we need to transform these data with an appropriate transformation before making strong inferences about what is going on in our dataset. 
