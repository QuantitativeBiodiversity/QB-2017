...
When scientists set up an experiment, they are inclined to take more than a single measurement on a subject (i.e., experimental unit).
...
RM-ANOVA explicilty accounts for this non-independence and allows one to test for the main effects of time and treatment along with the time *x* treatment interaction.  
...
The first treatment is the "Control" where rodents where free to move in and out of plots. 
...
In addition, the correlation statement specifies the the covariance structure that is associted with the repeated measures. 
Here, we use an autoregressive process of order 1, which means that an observation at *t* will be most influenced by observations at *t*-1.
Alternate covariance structures to consider are "unstructured" and "compound symmetry". 
...
As we have learned this semester, $\beta$-diversity examines biodiversity between samples.
...
Now we will explore temporal dimensions of $\beta$-diversity, which will examine changes in the abundance or presence-absence of species over time. 
Specifially, we introduce concepts and measures of turnover, rank shift, and rank change interval. 

Let us revisit the figure of annual rodent richness over time.
...
Turnover quantifies the similarity of species composition over time. 
Low turnover means that composition is relatively stable through time, while high turnover relfects dynamic community structure.
Turnover can be driven by the introduction and establishment of new speccies or the loss of a resident species. 
...
Here, we calculate the overall turnover with the follwing simple equation:
...
```{r}
# Use `join()` from `dply` to join the columns by shared year & plot type columns
...
Take a moment to reflect on what we just did. 
...
How different can communities become over time? 
And how fast does this happen? 
These are important questions for basic science, but are also important for topics in restoration ecology where legacy effects often impede management practicies. 
For example, in highly dynamic systems, turnover may saturate after a relatively short period of time while in more stable communities, 
it may take decades or centuries for communities to reach their maximum dissimilarity.
