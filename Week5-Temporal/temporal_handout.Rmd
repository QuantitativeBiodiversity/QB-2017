...
A time series can be broken down into different "categories", which can provide insight into what is contributing to patterns. 
The common categories of a time series decomposition are the trend, seasonality, cycle, and residual error.
...
Auto-regressive moving average (ARMA) models are commonly used to model time series. 
...
Similar to concepts of community-wide synchrony, here we present another metric, the variance ratio (VR). 
The VR allows us to test for positive or negative species covariance. 
The ratio of aggregate abundances compared to the sum of variances of individual species can generate three qualitatively different VR outcomes: 
When VR = 1 species do not covary; when VR > 1, species positively covary; when VR < 1 species negatively covary.
In order to test for significance, we will compare the observed variance ratio to a distribution of ratios calculated from randomized null communities. 
...
# Let's ungroup rich.all and regroup only by plot id 
...
### Can knit, but getting erros with portal.mean.rich.plot
### Can we add a lm(stabiltiy ~ richness) and abline to the plot?
