...
In addition to generating a lot of quality information, it was criticial for the Protal Project's data to be organized, accessible, and properly managed.  
...
These data are entered in *long format*, which means tha each row represents a unique observation (i.e., a trapped animal). 
...
In the following sections, we will demonstate how to manipulate the `portal` data set to carry out different types of analyses using functions from the `dplyr` and `tidyr` packages. 
These packages were especially designed for transforming, subsetting, and summarizing tabular data.
The functions contained in these two packages (and others like `apply()` and `aggregate()`) are fast and efficienct, and thus are preferred ways for manipulating data in R compared to other control-flow statements (e.g., for loops and while loops) that are commonly used in other programming languages. 
...
Because, the Chihuahuan Desert sits in a rain shadow created by the Sierra Madre mountains, the Portal site is an arid ecosytems with rain falling primarily during the months of June through October.
This variabiltiy in precipitation provides an opportunity for us to detect a signal of this *seasonality* in our dataset, while testing for long-term trends and forecasting rodent densities into the future.
...
Now we are going to use the `filter()` function in `dplyr` to pick a plot and calcualte rodent abundance for each season within a year.
...
The first thing we will do is create a time series data set, which identifies the start time and seasonality of our sampling, which is determined by the frequency argument. 
...
Play around with *n* to visualize the effects of the smooting window.
...
A common statistic for peforming exponential smoothing is Holt-Winters filtering.
In R, the `HoltWinters()` function in the `stats` package allows one to specify two parameters. 
The beta argument when set to FALSE peforms exponential smoothing, while the gamma argument is used to specify seasonality.
In the output, you will get an alpha parameter, which ranges from 0 to 1. 
Values closer to 1 indicate that the smoothing is influenced by current observations, while values closer to 0 indciate that more weight is placed on past observations. 
...
In the following section we will use the `decompose()` function to look at different decompposition categories.
...
Specifically, the ACF tells us about the lags of the forecast errors and thus tells abotu the MA component of the model. 
...
```{r}
acf(abund.ts) # autocorrelation function; decays geometically
```
...
After identifyng the ARIMA model, we can look at the diagnostics using `tsdiag` and then make forecasts about rodent densities into the future using the `predict()` function from the `arima` package. 
...
After this, `dplyr` allows one to apply functions to the split datset using functions like `summarise()`, which can return summary statistics (e.g., mean, min, max).
