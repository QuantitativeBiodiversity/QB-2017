...
After learning how to work with these data types, you will learn about contributed packages, which will aid in the visualization of data and with peforming descriptive and hypothesis-testing statistical procedures, including linear regression and ANOVA.
...
Thereofore, much of what we discussed in Week 0 about vectors translates directly into dealing with matrices. 
...
**Q1:** What does the `rnorm` function do? 
What are the arguments specifying? 
...
When should one use non-parameteric methods instead of parameteric methods?
...
During Quantitative Biodiversity this semester, you will be introducted to a few fundamental data structures that are critical for analyzing biodiversity data. 
The first of these is the **site-by-species matrix**. 
This site-by-speces matrix contains abundances, relative abundances, or the presence of species (or other taxonomic units) observed at different sampling sites. 
The site-by-species matrix is also very similar to other types of data that you might encounter if you are working with genomic, metagenomic, transriptomic, or metabolomic datasets. 
This means that, even if you don't care about biodiversity *per se*, the tools that you are learning in Quantitative Biodiversity can be transferred to address questions in other sytems!
