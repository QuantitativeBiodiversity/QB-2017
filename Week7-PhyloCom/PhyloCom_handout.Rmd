...
We will quantify phylogenetic diversity using analogues of $\alpha$- and $\beta$-diversity, and discuss key patterns that are commonly encountered in phylogenetic community ecology, such as over- and under-dispersion.
...
Darwin (1859) hypothesized that closely related species (e.g., species in the same genus) are more ecologically similar than distantly related species.
This led him to identify a paradox. 
...
On the other hand, the species must be sufficiently *unrelated* to resident taxa and thereby possess unique traits so that it can exploit unoccupied niches (Diez et al. 2008).
...
As a proxy, many scientists will use phylogeny as a proxy for traits. 
Based on these assumptions, ecologists have attempted to use phylogenetic information to gain insight into community assembly processes.
For example, if a community is comprised of closely related taxa (i.e. clustering), one might infer that enviromental filtering is acting on a set of shared functional traits; a scenario that is consistent with equalizing mechansisms of coexistence.
...
Here, we will use a source code file that includes a function for reading in the output files from the popular community sequencing software `mothur` (http://www.mothur.org/).
Take a moment to open the file and see how this file is formatted. 
...
We will revisit the environmental and community data that was used in the Spatial Diversity module. 
...
While use of the 16S rRNA gene indicates which taxa were present in our samples, use of the 16S rRNA transcripts indicates which taxa were metabolically active.
In this module we will focus on taxa that were present, which is how microbial communities are most often characterized.
We then used a `mothur` pipeline to quality-trim our data set and assign sequences to operational taxonomic units (OTUs).
In this Handout, we will use the DNA sequence data for making community phylogenetic inference about total (i.e., active + dormant) bacterial communities.

## 3) LOAD AND PROCESS DATA

First, let's load the environmental data. 
There are a few ponds with missing enviromental data. 
For simplicity, we are going to remove these ponds from our analyses. 
...
# Select DNA data: Use the `grep()` command to search for matches to a pattern within each element of a character vector
...
Because we have removed some samples from our larger "global" data set, there are some OTUs that are not found in our "reduced" data set.
In the following line of code, let's remove those taxa.

```{r}
comm <- comm[ , colSums(comm) > 0]
```

Now we are going to import the taxonomic information that is associated with the OTU data using the `read.tax()` from Source Code

```{r}
tax <- read.tax(taxonomy = "./data/INPonds.final.rdp.1.cons.taxonomy")
```

Now, we need to load and process the phylogenetic data.
There are a few steps involved in this process. 
First, we need to read in a file containing the aligned sequences. 
After a few other processing steps, we then make a neighbor-joining phylogenetic tree with a Jukes Cantor model of evolution.
You will remember from the previous phylogenetic traits module that the benefit of this approach is in its reduced computational time.
Generating trees of greater confidence with, for example, maximum likelihood analysis, can take a long time in the RStudio environment.
...
In order to analyze aspects of phylogenetic diversity, we first need to consider how to quantify it. 
Tucker et al. (2016) discuss several methods for quantifying phylogenetic diversity. 
In general, a community consisting of species that are less closely related has higher phylogenetic diversity. 
...
Let's compare PD estimates with S of our samples.
