...
This observation led him to the identifcation of a paradox. 
...
Species in a community must be ecologically similar enough to one another to have positive fitness in the enviroment (i.e., equalizing forces), but also be different enough (i.e., stabilizing niche differences) that they can coexist (e.g., see Leibold and McPeek 2006, Adler et al. 2007, HilleRisLambers et al. 2012).
...
In practice, it is challenging to measure ecologically relelvant traits of individuals in complex communities.
Therefore, many scientists will use phylogeny as a proxy for traits since phylogenetic information is becoming increasingly accessible. 
Ecologists have attempted to use phylogenetic information to gain insight into community assembly processes.
For example, if a community is dominated by closely related taxa (i.e. clustering), one might infer that enviromental filtering is acting on a set of shared functional traits; a scenario that is consistent with equalizing mechansisms of coexistence.
In contrast, if a community is comprised of more distantly related taxa (i.e., overdispersion), one might infer that competitive exclusion is an important assembly process; a scenario that is consistent with stabilizing mechanisms of coexistence.
It is important to note, however, that other mechanisms can give rise to patterns of phylogenetic clustering and overdispersion (e.g., see Mayfield and Levine 2010 for discussion).

~~~However, often times the assumptions that underlie much of current phylogenetic community ecology are observed, mainly: (1) traits related to competitive ability and species niches *are* phylogenetically conserved (and we discussed how to verify this last week), (2) competitive exclusion is more common than character displacement (meaning incompatible species are removed from the community), and (3) trait convergence is rare (among others: Gerhold et al. 2015).
When this is the case, we may be able to use patterns of phylogenetic relatedness to make deeper inference about the underlying processes that drive community structure (for general reviews, see Webb et al. 2002, Cavender-Bares et al. 2009, and Cadotte and Davies 2016).~~~
...
rm(list = ls())
getwd()
setwd("~/GitHub/QuantitativeBiodiversity/QB-2017/Week7-PhyloCom/")
```
...
In th chunk of code below, the `require()` function in `R` returns `TRUE` if the package was successfully loaded or `FALSE` if the package failed to load. 
...
Here, we will use a source code file that includes functions for reading in the output files that contain OTU and taxonomic information generated from `mothur` (http://www.mothur.org/), a popular software platform for analzying bacterial sequence data.
...
As a reminder, in 2013 we sampled more than 50 forested ponds located in Brown County State Park, Yellowood State Park, and Hoosier National Forest in southern Indiana. 
...
There are a few ponds with missing enviromental data. 
...
Here is a description of the column headers in `env`:
  ...
  + Sample_ID = unqiue pond identifier (e.g., "BC001" = pond 1 from BCSP)
  ...
  + Temp = surface water tempeature (C &deg;)
...
Therefore, we are going to use the `grep()` function, which is designed for matching patterns in string of characters.
Then we will use the `gsub()` function, which replaces matches in a string of characters, to clean up the rowname so it is more useful. 
...
Now we just need to clean up the `comm` dataframe a little bit. 
First we'll remove sites that contained in the `env` dataframe. 
Second, because we removed the RNA samples, there are likely to be some OTUs that are not found in the DNA sample. 
So, we'll remove any zero-abundance taxa. 
...
You can visualize the alignment in `ponds.cons`, but may also detect that the names associated with the FASTA entries are not very useful. 
So, we'll use `gsub()` again to clean things up. 
...
Next, we will import an archaeal sequence and specify this as our outgroup and then convert the alignment file to a DNAbin object using the `ape` package.
...
You will remember from the PhyloTraits module that the benefit of this approach is in its reduced computational time.
...
Next, we will specify the outgropu sequence, root the tree, and make a plot of the tree.
...
We will start by assessing the with-in site or $\alpha$-phylodiversity. 
...
In the following sections, we will introduce a few ways to calcaulte $\alpha$-phylodiversity.
...
It's important to note that PD, as implemented here, is based on the presence-absence of an OTU; it does not weight 
...
In addition to returning Faith's PD, the `pd()` function also returns species richness (S).
S is the same as observed richness ($S_{obs}), which we covered in the $\alpha$ diversity module. 
...
Overdispersed communities contain species tha re more distantly related than would be expected by chance. 
In this section, we introduce two ways of quantifying $\alpha$ phylodiversity based on the **dispersion** of species in a sample: the Net Relatedness Index (NRI) and the Nearest Taxon Index.
...
NRI is based on the mean phylogenetic distance (MPD), which is calculated based on the pairwise branch lengths in a sample that is contained in your phylogenetic resemblance matrix. 
With this information in hand, NRI is expressed as: - (`mpd.obs` - `mpd.rand.mean`) / `mpd.rand.sd` where `mpd.obs` is the observed MPD, essentially, the avarage value of the resemblance matrix for a given sample; `mpd.rand.mean` is the mean of the MPD values generated via randomization under a null model, and `mpd.rand.sd` is the standard deviation of the MPD values generated via randomization under a null model. 
...
As with Faith's PD, the randomization procedures are computationally intensive, so we are only going to peform a relatively small number of "runs".
...
Effectively, this is the sum of the minimum values for each colum (i.e. taxon) of the phylogenetic resemblance matrix.
...
In previous modules, we examined how taxonomic diversity changed across sites in response to environmental differences.
As you probably understand by now, examining taxonomic diversity does not reveal evolutionary relationships or the way in which environmental variables relate to shared evolutionary history.

### A. Phylogenetically Based Community Resemblance Matrix

As you may recall, a resemblance matrix is needed to quantify $\beta$-diversity for more than two samples. 
...
We need a similar process for quantifying phylogenetic $\beta$-diversity.
...
In the Spatial Diversity module, we demonstrated that there was substantial variation in environmental variables that are known to influence the structure and function of microbial communities.
...
Second, let's conduct a Mantel test to examine the potential correlation between UniFrac distance and environmental variation:
...
In the Spatial Diversity module, you learned that environments and communities can differ as a matter of geographic location.
You also learned that population densities are naturally aggregated and that patterns of diversity can arise from spatial autocorrelation and random distribution of individuals and species across space.
Here, we will add a phylogenetic dimension to the spatial diversity with the study of two phylogenetic patterns, i.e., the phylogenetic distance decay and the phylogenetic diversity-area relationship.
...
**THE FOLLOWING IS A COMBINATION OF CAVEATS AND FURTHER INSTRUCTION. NOT REALLY CONCLUSION MATERIAL.**

Phylogenetic relationships can be informative for understanding certain aspects of community ecology.
However, interpretations of phylogenetic patterns, just like taxonomic patterns, are dependent on spatial and temporal scale (Emerson and Gillespie 2008).
