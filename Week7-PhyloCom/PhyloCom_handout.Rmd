...
Based on these assumptions, ecologists have attempted to use phylogenetic information to draw gain insight into community assembly processes.
...
It is important to note, however, that other mechanisms can give rise patterns of phylogenetic clustering and overdispersion (e.g., see Mayfield and Levine 2010 for discussion).
...
Specifically, we amplified the 16S rRNA gene (i.e., the coding DNA sequence) and 16S rRNA transcripts (i.e., the corresponding RNA transcript) of bacteria using barcoded primers on the Illumina MiSeq platform.
...
~~~~ I haven't gotten through this yet, but do we end up using the RNA? If not, seems like we might as well not mention in this section; will make things easeir. 
...
~~~ Perhaps this is where we can mention RNA briefly and then say we're excluding. 
~~~ Might be good to talk about what we're doing with the grep/gsub functions. 
```{r}
...
After a few other processing steps, we then make a neihbor-joining phylogenetic tree with a Jukes Cantor model of evolution. 
...
There are many ways to quantify phylogenetic diversity (e.g., see Tucker et al. 2016). 
In general, a community consisting of species that are less closely related has higher phylogenetic diversity, all else equal. 
...
We'll transform our data by taking the natural logarithm of SR and PD so that the slope of the relationship gives us a power-law exponent, which describes how PD scales with SR.
