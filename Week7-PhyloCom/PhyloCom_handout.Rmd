...
Before moving forward, we need to do some additonal clean-up of the `comm` dataframe. 
...
It is important to note that PD, as implemented here, is based on the presence-absence of an OTU; that is, it does not weight the branches of common species more than the brances of rare species.
...
In order to draw strong conclusions about the phylogenetic diversity of a sample, it is usefuly to know something about how variable our metric is under null conditions. 
...
We will use some of these models for assesing the degree to which phylogenetic measures of $\alpha$ phylodiversity deviate from null expecations. 
...
In the $\beta$-diversity module earlier this semester, we learned about approaches to quantify how diversity changes among sites and how to link this to underlying enviromental gradients. 
Here, we will expand upon the concepts of taxonomic $\beta$-diversity by incoroporating phylogenetic information. 
By doing so, we can gain insight into landscape patterns of biodiveristy and the eco-evolutionary processes that may be shaping them. 
...
Some of the enviromental variables are almost perfectly correlated with one another since they are derived from the same information collected by a suite of field probes. 
...
Second, let's conduct a distance-based Redundance Analysis (dbRDA). 
...
# Permutation tests: axes and enviromental variables
...
In the Spatial Diversity module, you learned that communities can be affected by geography for different reasons (e.g., dispersal and evironmental conditions). 
...
Here, we will add a phylogenetic dimension to two fundamental patterns of biodiveristy: the distance decay (DD) relationship and the species area relationship (SAR).
...
Finally, let's test whether the slopes for taxonomic and phylogenetic DD are significantly different from one another using the permuation-based `diffslope()` function in the `simba` package:
...
In the following section, we calculate and plot the taxonomically based SAR for the pond data. 
