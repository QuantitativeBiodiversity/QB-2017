...
## OVERVIEW
Last week, we learned how to quantify relationships between taxonomic relatedness and functional trait similarity.
This week, we will use phylogenetic information to aid our understanding of patterns and processes in community ecology.
To accomplish this, we will introduce concepts linking evolutionary relatedness to community structure, we will quantify phylogenetic diversity using analogues of $\alpha$- and $\beta$-diversity, and discuss key patterns that are commonly seen in phylogenetic community ecology, such as over- and underdispersion.
...
Darwin (1859) hypothesized that closely related species (e.g., species in the same genus) are more ecologically similar than distantly related species are.
This led him to identify a paradox: successful invaders must be both (1) sufficiently *related* to resident taxa that they share ecologically-relevant traits that meet local environmental requirements, but also (2) sufficiently *unrelated* to exploit currently unoccupied niches, allowing them to invade the community (Diez et al. 2008).
This parallels the current thinking in community ecology that species in a community must be ecologically similar enough to one another to have positive fitness in the environment (i.e., equalizing forces), also different enough (i.e., stabilizing niche differences) that they can coexist (e.g., see Leibold and McPeek 2006, Adler et al. 2007, HilleRisLambers et al. 2012).
The assumption that more closely related species share more similar functional traits may not always be valid though, which makes interpreting patterns of phylogenetic clustering and overdispersion less straightforward than they traditionally have been (e.g., see Mayfield and Levine 2010 for discussion).
...
# To keep things simple, we'll remove sites with incomplete data
env <- na.omit(env)
```
