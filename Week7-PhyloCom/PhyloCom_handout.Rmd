...
For example, if a community is dominated by closely related taxa (i.e. clustering), one might infer that enviromental filtering is acting on a set of shared functional traits.
...
Therefore, we are going to use the `grep()` function, which is designed for matching patterns in string of characters, or "regular expressions".
...
Next, we will specify the outgroup sequence, root the tree, and plot the tree.
...
It's important to note that PD, as implemented here, is based on the presence-absence of an OTU; that is, it does not weight the branches of common species more than the brances of rare species.
...
Now that we have a way to quantify PD, how do we know if our observed values really reflect high phylogenetic diversity or are merely an artifact of high species richness?
In order to say whether a site has unusually high or low phylogenetic diversity, we need to know something about how variable our metric is under null conditions. 
...
Understanding how phylogenetic relationships change across sites may be useful for identifying whether groups of sites differ because of differing local conditions (e.g., pH or primary productivity) or historical factors (e.g., in situ radiations due to dispersal limitations). 
As with the discussions about phylogenetic alpha diversity, this is a highly active research field.
You are encouraged to seek out additional information to learn more about how phylogenetic information can aid your understanding of among-site variability in phylogenetic and taxonomic diversity. 
...
Let's calculate the phylogenetic community resemblance matrices for our pond data set. 
...
Now that we have our phylogenetically based community resemblance matrix, we can visualize phylogenetic diversity among samples using the same techniques that we used in the $\beta$-diversity module.
...
# These are highly correlated variables, so let's remove them to reduce overfitting
...
Last, let's conduct a distance-based Redundance Analysis (dbRDA). 
...
Here, we will add a phylogenetic dimension to spatial diversity with the study of two phylogenetic patterns: the phylogenetic distance decay and the phylogenetic diversity-area relationship.
...
That is, communities located near one another should be more similar to one another than to distant communities.
Here, we will test to what degree spatial autocorrelation can also affect phylogenetic DD.  

First, we will calculate pairwise geographic, taxonomic, and phylogenetic distances between ponds.
...
This phylogenetic diversity-area relationship (PDAR) is analogous to the species-area relationship (SAR) that we learned about in the Spatial Diversity module.
  ...
  for (i in num.ponds) {   
    ...
    for(j in 1:100){
      ...
      for (k in pond.sample) { # Loop through each randomly drawn pond
        ...
        cum.abs <- cum.abs + as.numeric(comm[k, ]) # combine comm data for each site
...
*Is there an simple example to clearly demonstrate how this can occur?*
