...
rm(list = ls())
getwd()
setwd("~/GitHub/QuantitativeBiodiversity/QB-2017/Week3-Beta/")
```
...
Also, you can use the the dollar sign (`$`) after the list name (`doubs`) to access objects within the list (e.g., `env`). 
In this example, we introduce a new data structure that is similar in ways to the site-by-species matrix.
For each site we have environmental data where species information (i.e., presence-absence or abundance) were measured.
We will call this the **site-by-environment** matrix, which we will use later to explain variation in $\beta$-diversity.
To access the site-by-environment matrix in this case, you would pass `doubs$env` to the R console.
...
Often, the objects we want to visualize are samples in "species space", but with a simple transpose, one can look at speices plotted in "sample space".
...
The relationships between individual species and groups of sites (e.g., habitat quality, disturbance regime, land use) can be informative for many ecological questions. 
For example, the presence or abundance of certain invertebrate taxa is often used to provide an indication of stream water quality. 
To determine whether any fish species in our dataset can be used as indicators of habitat quality, we can calculate each species’ Indicator Value (IndVal) (Dufrene and Legendre 1997, De Caceres and Legendre 2009) for each habitat grouping, and then run a permutation test to evaluate its significance.
Significant IndVal scores close to 1 suggest that the presence of a particular species is a strong indicator of a certain site group.
...
Alternatively, we may wish to learn more about the habitat preferences (i.e. degree of species preference for the target group over the other groups) of each fish species.
In this case, we can calculate the phi coefficient of association ($r_{\phi}$), the Pearson correlation between a vector of sites indicating presence (1) or absence (0) of the species, and a vector a sites indicating whether the site is within (1) the group or not (0). 
This approach can also be extended to abundance data, in which case the index is called the point-biserial correlation coefficient ($r_{pb}$) where the 1s of the species-site vector are replaced with species abundances.
Values of the phi coefficient can range from -1 (suggesting strong avoidance/exclusion from a group) to 0 (suggesting no preference among groups) to +1 (suggesting strong perference for a group). 
```{r, results="hide", message=FALSE}
...
To learn more about species-site group associations, see De Caceres and Legendre (2009).
...
It looks like many of our environmental variables are highly correlated with one another and could lead to model overfitting. 
...
# so the axes suggest this is a MDS (i.e., PCoA)
...
From the above analyses, we have shown that spatial variation in fish community structure in the Doubs River can be explained by environmental variables. 
...
In general, we will create a distance matrix between our sites, truncate sites that are far away from each other (hence the "Neighbor Matrices"), then create spatial eigenvalues from this truncated matrix (hence the "Principal Coordinates"). 
Our goal is to represent "space" as some combination of these eigenvectors, so that we can indentify spatial structure in species abundances on broad, intermediate, and fine spatial scale.
...
Now, we need to calculate the fractions of variation explained by space alone, environment alone, both space and environment, and neither space nor environment.
...
# Note that the initial partitions agree (X1, X2) but that
# the varpart() function applies a correction to R2 after partitioning
# These adjustments minimize the shared fraction [b] and tend to inflate [d]
par(mar = c(2,2,2,2))
plot(doubs.varpart)
text(1, 0.25, "Space")
text(0, 0.25, "Env")
...
***Question ***: Based on the three criteria described above, does the PCoA do a good job of explaining variation in the `doubs` data set? Please justify.
