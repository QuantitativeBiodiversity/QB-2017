...
Up to this point, Quantitative Biodiversity has focused entirely on quantifying aspects of taxonomic diversity. This has included patterns and concepts of commonness and rarity, and diversity across space and time. However, measures of taxonomic diversity overlook the evolutionary relatedness of taxa and their traits. ~~Since evolution can be broadly defined as descent with modification, it is likely that recently diverged taxa have a greater degree of similarity in their traits relative to taxa that diverged a long time in the past.~~

To think about about how taxa are related and how that relates to ecologically-relevant traits we will have to construct evolutionary trees (i.e., phylogenies), the fundamental ~~data~~ evolutionary structure that allows us to statistically examine the evolutionary differences among and between taxa. By building your tree you will be able to gain evolutionary insight into patterns and processes of biodiversity (e.g., diversification, adaptation).
...
We extracted DNA from all of the isolates and amplified the 16S rRNA gene, which is a commonly used "marker" for delineating microbial taxa and examining the phylogeny of bacteria.
...
While neighbor joining if very fast and has the convenient property of returning the correct tree for a correct distance matrix, we are rarely in the position where we know the correct distance matrix. 
...
From the section above where we described the data, you will recall that we were interested in testing for an ecological trade-off related to species traits.
