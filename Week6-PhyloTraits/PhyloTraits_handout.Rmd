...
## OVERVIEW
** The following builds on what the students have learned, rather than opposes it.**
So far, we have focused on quantifying aspects of taxonomic diversity.
This has included patterns and concepts of commonness and rarity, and diversity across space and time.
In this module will consider the evolutionary relatedness of taxa by introducing basic concepts of phylogenetic diversity.

**There needs to be more to the overview. Something like the following:**
Phylogenetics is the study of evolutionary relationships and it relies heavily on the concept of phylogenetic trees.
Phylogenetic trees are models or hypotheses for how species and higher level taxa are related. 
Phylogentic trees are represented with branching diagrams that follow rules such as parsimony, branching, and rootedness.
You will be introduced to these rules and other concepts below, as you learn to to test for the signal of phylogenetic relatedness.


**THE STUDENTS ARE THROWN INTO PHYLOGENETIC ANALYSIS WITHOUT ACTUALLY INTRODUCING THEM TO BASIC CONCEPTS, RULES, ETC.
IF WE'VE GONE TO THE EXTENT TO EXPLAIN WHAT SPECIES RICHNESS IS, YOU SHOULD DEFINITELY EXPLAIN THE BASICS OF A PHYLOGENETIC TREE.
AT LEAST, FOR CONTEXT. AT MOST, BECAUSE I CAN THINK OF 4 STUDENTS AND 2 CO-INSTRUCTORS WHO DON'T REALLY KNOW OR PROBABLY CAN'T RECALL THE PROPERTIES AND PRINCIPLES OF PHYLOGENIES** --- how's about them all caps ;)

After completing this exercise you will be able to:

1.  create phylogenetic trees to (explore, visualize, ?) evolutionary relationships
2.  map traits onto phylogenetic trees to (visualize, understand, ?) 
3.  test for phylogenetic signal within trait distributions and trait-based patterns?
...
We will rely heavily on the `ape` package (http://goo.gl/nExs0), which performs a wide range of phylogenetic analyses. 
We will also use `seqinr` for aligning sequences and `geiger` for testing for phylogenetic signal.
We will also align sequences using the program `muscle` through the terminal.

**The students will see the following as a laundry list of packages for complicated things they don't know and aren't given any relevance for, i.e., how they relate to phylogenetics. Perhaps, explain the packages when they're introduced below.**
You may also want to check out the `phangorn` package (http://goo.gl/iWU6Vt), which is good for conducting maximum likelihood analyses in R. It is also worth looking into the Randomized Axelerated Maximum Likelihood (RAxML) package for generating maximum likelihood values with bootstrap support (http://sco.h-its.org/exelixis/web/software/raxml/index.html). <-- Every time a student's head explodes, an angel gets it wings.

For large numbers of taxa, programs like RAxML may be too slow. Here you can use the program FastTree to generate approximately-maximum-likelihood phylogenetic trees, sacrificing exactness for speed. However, both RAxML and FastTree will have to be run in the terminal. **<-- Perhaps better included when the packages are actually being used.**

**Suggested -->** We will describe the packages in greater details in the sections below.
...
**I CUT AND SCRAMBLED THE ORDERING FOR MORE LINEAR ORGANIZATION**
In this week's module, we will explore phylogenetic relatedness in regards to ecological traits.
We will use a dataset that is well-suited for this purpose, that is, molecular surveys and environmental data from two lakes in ...
...We isolated 39 strains of bacteria from two lakes.
Originally, these data were gathered to explore (check w/ Jay)... after identifying these bacteria based on their 16S rRNA gene sequences, we measured their growth on 18 different forms of phosphorus. 
Here, we will take a phylogenetic approach to mapping phosphorus resource use onto a phylogenetic tree while testing for specialist-generalist trade-offs.

**EVERYTHING below reads like a to-do list. No reasons given. No context given. No ideas presented. Nothing defined. The following directions are, basically, best suited for people who already understand phylogenetics and who know why to do and how to do these analyses.**

## 3) SEQUENCE ALIGNMENT 

**To do:** After generating or retrieving sequence data, the first step of a phylogenetic analysis is to perform an alignment. 
...
**To do:** We will read in a FASTA-formatted file into the `muscle` alignment function.
...
**To do:** It is good practice to view your sequences after alignment. 
...
**To do:** Once you have aligned your sequences, the next step is to construct a phylogenetic tree.
...
Often a nearest neighbor joined tree is not good enough. While neighbor joining if very fast and has the convenient property of returning the correct tree for a correct distance matrix, we are rarely in the position where we know the correct distance matrix. Instead a maximum likelihood (ML) tree is often a superior choice, as it is built on the robust statistical procedure of ML (i.e. the process of finding the parameter value that maximizes the likelihood of the data). In addition, ML has the advantage of being the estimation method that is least affected by sampling error, being fairly robust to the assumptions of a particularly model, allows you to compare different tree topologies, and uses all the information in a sequence (as opposed to just distance). 
...
With this information we will visualize the functional traits (i.e., growth on different phosphorus) in a phylogenetic context by mapping them onto the neighbor joining tree (with the F84 DNA substitution model) that we created. 
...
In addition, these patterns of phylogenetic signal provide insight into ecological and evolutionary processes giving rise to the **clustering** or **overdispersion** of traits, which are sometimes associated with environmental filtering and competitive repulsion, respectively.
...
In ecology, dispersion is commonly used to measure of how things (e.g., traits or species) are distributed in space.
