...
Because it can take a long time to estimate a maximum likelihood phylogeny in R, we've blocked out the code. Feel free to make your own maximum likelihood phylogeny on your own.  
...
We have also included a short script in the folder `bash` under Week6 that will allow you to generate a maximum likelihood tree on the IU computer cluster Mason using the program RAxML. Generating a maximum likelihood tree is computationally intensive, so if you need a maximum likelihood tree it's often easier to run it on a desktop or a computer cluster rather than your laptop. If you are an IU student, postdoc, or faculty member you can register for an account on Mason. See the IU Mason webpage for more information (https://kb.iu.edu/d/bbhh).
