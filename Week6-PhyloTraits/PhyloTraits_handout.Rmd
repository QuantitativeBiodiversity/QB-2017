...
Up to this point, Quantitative Biodiversity has focused on quantifying aspects of taxonomic diversity.
This has included patterns and concepts of commonness and rarity, and diversity across space and time.
Measures of taxonomic diversity, however, overlook the evolutionary relatedness of taxa and the traits that they possess.
In this module we introduce the basic concepts and tools of phylogenetics to gain evolutionary insight into patterns and processes of biodiversity.
You will learn how to align sequences and make phylogenetic trees using different evolutionary models. 
We will then map functional traits onto phylogenetic trees and peform tests to determine and correct for phylogenetic signal in a dataset. 
...
In terms of functional traits, for each strain, we measured growth characteristics, including the maximum growth rate $\u$max, lag time, and yield, on 18 different phosphorus sources.
...
~ ## this is oddly stated ## You will often find some function that you want to use that you either can't find in R or code up yourself. However, you can still run all your code in R, you just have to specify what language you'll be using. Here we will be using the `muscle` program in the command language interpreter Bash~.

Bash is the language that you are using on the lab Macs. 
Using Bash, we'll call the program `muscle`, read in our FASTA file, and create a FASTA-aligned output file (with an .afa extension).  
