...
Functional traits are morhpological, behavioral, or physiological characteristics of an individual that affect its peformance or fitness under a set of enviromental conditions. 
...
Second, "effect traits" are characteristics of an indvidual that can alter the enviroment (Lavorel and Garnier 2002).
...
Therfore, it is possible to detect trait-envriroment and trait-function relationships simply becuase an assemblage is comprised of closely related taxa. 
We can not only test for this so-called **phylogenetic signal**, but also correct fot it. 
...
In the following sections, we will provide an introduction to contructing phylogenetic trees.
...
rm(list = ls())
getwd()
setwd("~/GitHub/QuantitativeBiodiversity/QB-2017/Week6-PhyloTraits")
```
...
One lake has extremeley low concentrations of phosphorus (i.e., oligotrophic) while the other lake is adjacent to a bird sanctuary and receives high inputs of phosphorus (i.e., eutrophic).
...
With this informaiton, we can measure the **niche breadth** of each strain and consider this a functional trait. 
...
One of the first steps in peforming a phylogenetic analysis is to align the sequences that have been obtained from your samples. 
...
Local aligment is typically a preferable approach and uses algorithms that initiate alignment by identifynig regions in a sequence that have a high degree of similarity.
In the example below, we will be using a local alignment approach. 

### A.  Examining a FASTA-File

WS: ~ get rid of `muscle` function/package references throughout ~
...
WS: ~ two options; either nano or less function or the cat function
...
After peforming the alignment, it is good practice to view your aligned sequence file. 
...
~WS: Do we want to include the text in the next three sentences or leave for student interpretation?
...
It is not possible to cover all of these topics in Quantitative Biodiveristy, but we try to highlight some of the major concepts in the following sections, including the table below:
...
WS: ~~~ **perhaps add more on what nj does?
...
WS: ~~~ **We should probably introduce the `phylo` object in R, since it's the common currency of all these phylogenetic packages and the phylocom stuff next week.**
...
In addition, raw distance measures do not take into account substitution biases for one particular nucleotide (e.g., ...WS ~~~~). 
...
WS: ~~They might ask what the point of this is, so we should tell them. ~~
...
The symmetric difference is simply the number of partitions (WS: ~~~~what's a "partition"?) that are in one tree and not the other. 
...
This function should look familiar, since it is just the squared Euclidean distance between two trees. 
...
WS:~~~How do I interpret 0.0438? What is this? Is this good or bad? ~~~~

### D) MAXIMUM LIKELIHOOD TREE
WS" ~~~~Often a nearest neighbor joined tree is not good enough. **<-- REVISE/CLARIFY**
...
In addition, ML has the advantage of being the estimation method that is least affected by sampling error, is fairly robust to the assumptions of a particularly model, allows you to compare different tree topologies (**this isn't described until below**), and uses all the information in a sequence (as opposed to just distance). 
However, ML estimation is a computationally intensive process that can require a lot of time. 

This is because ML estimation often must consider all different ways that you can make a tree (**This is not the definition of topology, i.e., the overall branching strucature:**) http://www.ebi.ac.uk/training/online/course/introduction-phylogenetics/what-phylogeny/aspects-phylogenies/topology) 

...(i.e., the topology of the tree) and find the best edge lengths for each topology.
...
To peform bootstrapping, we will select a subset of columns from our alignment randomly with replacement. 
...
R is useful for a lot of things, but it is not optimized for phylogenetic reconstruction and thefore can be extremely slow. 
...
With this information we will visualize the functional traits (i.e., growth on different phosphorus) in a phylogenetic context by mapping them onto the neighbor joining tree (with the F84 DNA substitution model: ~~~WS**see table X**~~~) that we created. 
...
From the section above where we described the data, you will recall that we were interested in testing for an ecological trade-off related to species traitsWS:~~~~ <- I don't recall this being mentioned.
...
WS:~~~~So what do we do with `p.phylosignal`?~~~~
...
WS:~~~~Is there a non-obtrusive way to print the output of some of these functions to discuss what they mean at all? e.g., using pander for pretty print of tables
...
```
~~~~WS: I get a value of zero here because `length(dists$BM[dists$BM > out$lr])` is zero for some reason What does a value of 0 mean here?~~~
...
WS:~~~~This section looks cool, but the switch to a dataset containing mammalian metabolic rates felt abrupt:

To show how to perform a phylogenetic regression we will examine the scaling relationship (i.e. power law) between body mass (M) and Basal Metabolic Rate (BMR), the minimal rate of energy expenditure per unit time by endothermic animals at rest. 
