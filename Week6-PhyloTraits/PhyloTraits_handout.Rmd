...
Up to this point, Quantitative Biodiversity has focused entirely on quantifying aspects of taxonomic diversity. This has included patterns and concepts of commonness and rarity, and diversity across space and time. However, measures of taxonomic diversity overlook the evolutionary relatedness of taxa and the traits that they possess. Since evolution can be broadly defined as descent with modification, it is likely that recently diverged taxa have a greater degree of similarity in their traits relative to taxa that diverged a long time in the past. 

To think about about how taxa are related and how that relates to ecologically-relevant traits we will have to construct evolutionary trees (i.e. phylogenies), the fundamental data structure that allows us to statistically examine the evolutionary differences among and between taxa. By building your tree you will be able to gain evolutionary insight into patterns and processes of biodiversity.
...
Functional traits refer to physiological, behavioral, and morphological characteristics that inluence the peformance of a taxon. 
We will use a data set that contains information on 39 strains of freshwater bacteria. 
The bacteria were isolated from two very different lakes.
One of the lakes has extremeley low concentrations of nutrients (i.e., oligotrophic) while the other lake is adjacent to a bird sanctuary and receives very high inputs of nutrients (i.e., eutrophic).
We extracted the DNA from all of the isolates and amplified the 16S rRNA gene, which is a commonly used "marker" for examining the phylogeny of bacteria.
In terms of functional traits, for each strain, we measured growth characteristics, including the maximum growth rate $\mu_{max}$, lag time, and yield, on 18 different phosphorus sources.
...
One of the first steps in peforming an phylogenetic analysis is to to align the sequences that have been obtained from your samples. 
The sequences can represent nucleotides (DNA or RNA) or proteins (amino acids).
The alignment process uses an algorithm to arrange seqeunces relative to one another based on conserved (i.e., non variable) regions.
...
Not every program is available as an R package and will need to be run in the terminal. However, you can still run all your code in R, you just have to specify what language you'll be using. Here we will be using the `muscle` program in the command language interpreter `Bash`, the language that you are using on the lab Macs.  Using Bash, we'll call the program `muscle`, read in our FASTA file, and create a FASTA-aligned output file (with an .afa extension).  
...
This may not be practical for large databases, but in our example, the alignment only contains sequences for 39 bacterial taxa plus a distantly related reference sequence that we will use as an **outgroup**. 
