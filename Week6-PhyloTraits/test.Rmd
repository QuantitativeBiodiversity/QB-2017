...
package.list <- c('ape', 'seqinr', 'phylobase', 'adephylo', 'geiger', 'picante', 'stats', 'RColorBrewer', 'caper', 'phylolm', 'pmc')
...
```{bash, results = 'hide'}
muscle -in ./data/p.isolates.fasta -out ./data/p.isolates_test.afa
```
...
## 7) QUANTIFYING TRAIT EVOLUTION

talk about how things are related, need to correct for  

### A. Correcting for the underlying phylogeny


```{r}
data(flowerSize)
data(flowerTree)
flowerSizeTest <- flowerSize
flowerSizeTest$test_trait <- c(-0.3866749,  0.9362907,  0.9062907,  0.4254651,  1.5862944,  0.9931472,  1.3986123,  0.3654651,  1.386123,  0.6331472,  0.8662907,  1.2986123,  0.454651,  0.454651,  1.094379,  0.631472,  1.986123,  2.25851,  0.631472,  0.962907,  0.631472,  5.203118,  4.9416424,  1.527630,  5.647860)

results <- phylolm(log_transformed_size ~ test_trait, data = flowerSizeTest, 
                   flowerTree, model = 'lambda', boot = 1000)
plot(flowerSizeTest$log_transformed_size, flowerSizeTest$test_trait,  las = 1)
abline(a = results$coefficients[1], b = results$coefficients[2])
