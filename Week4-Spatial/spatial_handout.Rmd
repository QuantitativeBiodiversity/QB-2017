...
package.list <- c('vegan', 'sp', 'geoR', 'rgdal', 'raster', 'RgoogleMaps', 'maptools')
...
These variables include elevation (m), geographical coordinates (lat-long), temperature (C), pond diameter(m), pond depth (m), redox potential (ORP), specific conductivity or SpC (\mu S/cm), dissolved oxygen (mg/L), total dissolved solids (g/L), salinity (p.s.u.= ppm), color - measured at absorbance = 660; an estimate of carbon in the water sample, chlorophyll *a* (\mu g/ml), dissolved organic carbon (mg/L), dissolved organic nitrogen (mg/L), and total phosphorus (\mu g/L).
...
### We're now adding citations to an in-Handout Reference section at the bottom of document
### Tobler 1970 seems to be first one. 
...
# Identify the current projection and datum
proj4string(xy) <- CRS("+proj=longlat +datum = NAD83") 

# Transform the projection and data, so we can get meaningful distances
...
### we're taking questions out of handout, right?
