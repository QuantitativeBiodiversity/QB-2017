...
**A GLOSSARY is provided at the end of this document for basic definitions of GIS terms.**
...
Typically, we say that a variable ceases to be autocorrelated when the semivariagram asymptotes.
...
OLS <- lm(struc ~ env)
summary(OLS) # print regression results to the screen
abline(OLS, col="red4")
...
We can refer to this pattern as the spatial abundance distribution and can examine it in regards to all individuals of a community, to all individuals of a particular species, or to any other subset of particular interest (e.g., rare species, common species).
...
Arrhenius (1921) first described the general form of the *species-area relationship (SAR)* as a power-law: $S = cA^{z}$ where S is species richness and A is area.
...
```{r, results = 'hide', message = FALSE, warning = FALSE}
community <- c() # an initial empty community
