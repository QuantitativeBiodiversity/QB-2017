...
From there, we will discusss ways to integrate richness and evenness, which will include univariate metrics of diversity and an investigation of the **species-abundance distribution (SAD)**.
...
In this excercise, we will use a popular R package called `vegan`, which contains tools commonly used in ecological research, including analyses of $\alpha$-diversity.
...
More information on the BCI dataset can be found elswhere (http://ctfs.arnarb.harvard.edu/webatlas/datasets/bci/).
...
The version of the BCI site-by-species matrix provided in `vegan` contains the abundances of more than 200 tree species (i.e., >200 columns) for each of 50 sites (i.e., 50 rows).
...
On ther other hand, *Apeiba glabra* is found at all eight sites and is more abundant than any of the other three species that we indexed in the data frame.
...
Functions are central to the use of R and most any other computing software.
In short, functions are pieces of code that operate on one or more variables or objects (e.g., scalars, lists, matrices, etc.).
In fact, you've already used several native R functions, e.g., dim(BCI).

Instead of repeatedly rewriting code to calculate richness, we define a function called `S.obs` that calculates richness of a given vector (i.e., `x`).
...
### How Well Did You Sample Your Site?
...
In the following R chunk, we load the bacterial dataset as a matrix and identify a sample (i.e., site) for subsequent analysis.
...
*WE SHOULD ELABORATE ON WHAT THIS S.chao1 FUNCTION DOES*
...
*WE SHOULD ELABORATE ON WHAT THIS S.chao2 FUNCTION DOES*
...
When applied to species abundances, this means that a histogram of species abundances (i.e., the species-abundance distribution; SAD) should look like a bell curve.
